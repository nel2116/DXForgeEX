    ...
    static class ContentInfoCache
    {
        ...
        public static void Reset(string projectPath)
        {
            lock (_lock)
            {
                ...
                if (!string.IsNullOrEmpty(projectPath))
                {
                    ...
                    _cacheFilePath = $@"{projectPath}.DXForge\ContentInfoCache.bin";
                    LoadInfoCache();
                }
            }
        }
        ...
        private static void SaveInfoCache()
        {
            ...
            catch (Exception ex)
            {
                ...
                Logger.Log(MessageType.Warning, "コンテンツブラウザのキャッシュファイルの保存に失敗しました。");
                _contentInfoCache.Clear();
            }
        }
        ...
        private static void LoadInfoCache()
        {
            ...
            {
                Debug.WriteLine(ex.Message);
                Logger.Log(MessageType.Warning, "コンテンツブラウザのキャッシュファイルの読み込みに失敗しました。");
                _contentInfoCache.Clear();
            }
        }
