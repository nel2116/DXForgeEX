...
namespace dxforge::graphics::d3d12::content
{
	namespace
	{
		...
		utl::free_list<u32>                             descriptor_indices;		///< ディスクリプタのインデックス
		...
		std::mutex                                      pso_mutex{};			///< パイプラインステートオブジェクト用ミューテックス
		...
		public:		// パブリック関数
			...
			explicit d3d12_material_stream(std::unique_ptr<u8[]>& material_buffer, material_init_info info)
			{
				...
				const u32 buffer_size{
					...
					sizeof(material_surface) + 									// PBR Materialのプロパティ
		...
		private:	// プライベート関数
			...
			u8* _buffer;							///< バッファ
			material_surface* _material_surface;	///< PBR Materialのプロパティ
		...
		id::id_type create_root_signature(material_type::type type, shader_flags::flags flags)
		{
			...
			case material_type::opaque:
			{
				...
				root_signature = d3dx::d3d12_root_signature_desc{ &parameters[0], _countof(parameters), get_root_signature_flags(flags), &samplers[0], _countof(samplers) }.create();
...
#pragma intrinsic(_BitScanForward)
		...
		d3d12_texture create_resource_from_texture_data(const u8* const data)
		{
			...
			const bool is_3d{ (flags & dxforge::content::texture_flags::is_volume_map) != 0 };
			...
			assert(!(flags & dxforge::content::texture_flags::is_cube_map && (array_size % 6)));
			...
			if (flags & dxforge::content::texture_flags::is_cube_map)
			{
	...
	namespace submesh
	{
		...
		id::id_type add(const u8*& data)
		{
			...
			// NOTE: 要素のサイズは、位置のみの頂点フォーマットの場合、0でもよい。
	...
	namespace render_item
	{
		...
		id::id_type add(id::id_type entity_id, id::id_type geometry_content_id, u32 material_count, const id::id_type* const material_ids)
		{
			...
			dxforge::content::get_submesh_gpu_ids(geometry_content_id, material_count, gpu_ids);
		...
		void get_d3d12_render_item_ids(const frame_info& info, utl::vector<id::id_type>& d3d12_render_item_ids)
		{
			...
			dxforge::content::get_lod_offsets(frame_cache.geometry_ids.data(), info.thresholds, count, frame_cache.lod_offsets);
			...
			for (u32 i{ 0 }; i < count; ++i)
			{
				...
				const dxforge::content::lod_offset& lod_offset{ frame_cache.lod_offsets[i] };
