...
namespace dxforge::graphics::d3d12::gpass
{
	namespace
	{
		...
		d3d12_depth_buffer gpass_depth_buffer{};									// GPassのデプスバッファ
		math::u32v2 dimensions{ initial_dimensions };								// サイズ

		ID3D12RootSignature* gpass_root_sig{ nullptr };								// GPassのルートシグネチャ
		ID3D12PipelineState* gpass_pso{ nullptr };									// GPassのパイプラインステート

#if _DEBUG
		constexpr f32 clear_value[4]{ 0.5f, 0.5f, 0.5f, 1.0f };						// クリアカラー(デバックでは灰色)
#else
		...
		bool create_buffers(math::u32v2 size)
		{
			...
			desc.Alignment = 0;	// NOTE: 0は64KB（MSAAの場合は4MB）と同じ
			...
			desc.MipLevels = 0;	// すべてのミップ・レベルのためのスペースを作る
			desc.SampleDesc = { 1, 0 };
			desc.Width = size.x;
	...
	const d3d12_render_texture& main_buffer(void)
	{
		return gpass_main_buffer;
	}

	const d3d12_depth_buffer& depth_buffer(void)
	{
		return gpass_depth_buffer;
	}

	/// @brief バッファーのサイズを設定する
	/// @param size サイズ
