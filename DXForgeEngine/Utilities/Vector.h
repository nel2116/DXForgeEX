...
namespace dxforge::utl
{
	...
	public:		// ====== パブリック関数 ======
		...
		{
			// std::is_copy_constructible<T>::value == true であることを確認する。
			static_assert(std::is_copy_constructible<T>::value, "Type must be copy-constructible");

			if (new_size > _size)
			{
				// 新しいサイズが現在のサイズより大きい場合、メモリを確保し、要素を追加する。
				reserve(new_size);
				while (_size < new_size)
				{
					emplace_back(value);
		...
		constexpr void reserve(u64 new_capacity)
		{
			...
			{
				// NOTE: realloc()は、新しいメモリ領域が割り当てられると、バッファ内のデータを自動的にコピーする。
				void* new_buffer{ realloc(_data, new_capacity * sizeof(T)) };
				assert(new_buffer);
				if (new_buffer)
				{
					// 新しいメモリ領域が割り当てられた場合、ポインタを更新する。
	...
	private:	// ====== メンバ関数 ======
		...
		/// @brief 配列のデストラクタを呼び出す。
		/// @param first デストラクタを呼び出す最初の要素のインデックス
		/// @param last デストラクタを呼び出す最後の要素のインデックス
		/// 配列のデストラクタを呼び出す。 デストラクタは、要素が破棄されるときに呼び出されます。
